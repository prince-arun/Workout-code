...
function findElement() {
  let findRating = superMarket.find((rate) => {
    return rate.rating >= 3;
  });
  document.write(`<h2> The element with rating >= 3 </h2>`);
  for (const key1 in findRating) {
    document.write(`<b>${key1}</b>\t : ${findRating[key1]}<br>`);
    // document.write(`<br>`);
  }
  document.write("<hr>");
  let findVeg = superMarket.find((veg) => {
    return veg.type === "vegetable";
  });
  document.write(`<h2> The element with type vegetable </h2>`);
  for (const key2 in findVeg) {
    document.write(`<b>${key2}</b>\t : ${findVeg[key2]}<br>`);
    //document.write(`<br>`);
  }
  document.write("<hr>");
  let findPrice = superMarket.find((dollar) => {
    return dollar.price < 15;
  });
  document.write(`<h2> The element with price below 15 </h2>`);
  for (const key3 in findPrice) {
    document.write(`<b>${key3}</b>\t : ${findPrice[key3]}<br>`);
    //document.write(`<br>`);
  }
  document.write("<hr>");
  let findLemon = superMarket.find((lemon) => {
    return lemon.title.includes("Lemon");
  });
  document.write(`<h2> The element with title </h2>`);
  for (const key4 in findLemon) {
    document.write(`<b>${key4}</b>\t : ${findLemon[key4]}<br>`);
    //document.write(`<br>`);
  }
  document.write("<hr>");
  let findMeasure = superMarket.find((measurement) => {
    return measurement.height == 450 && measurement.width == 450;
  });
  document.write(`<h2> The element with Height and Width </h2>`);
  for (const key5 in findMeasure) {
    document.write(`<b>${key5}</b>\t : ${findMeasure[key5]}<br>`);
    //document.write(`<br>`);
  }
}
findElement();
