...
let filterDairy = superMarket.filter(
  (filterType) => filterType.type === "dairy"
);
let avgDairyRating = filterDairy.reduce(
  (acc, rate) => acc + rate.rating / filterDairy.length,
  0
);
document.write("<h3>The Average Rating of the Dairy product is</h3> <br>");
document.write(`<b>${avgDairyRating}</b><br>`);
document.write(
  "----------------------------------------------------------------------"
);
// 2. Filter the elements of type: ‘vegetable’ and calculate averagePrice
let filterVeg = superMarket.filter(
  (vegetable) => vegetable.type === "vegetable"
);
let avgVegPrice = filterVeg.reduce(
  (acc, veg) => acc + veg.price / filterVeg.length,
  0
);
document.write("<h3>The Average Price for the Vegetable product is</h3> <br>");
document.write(`<b>${Math.round(avgVegPrice)}</b><br>`);
document.write(
  "----------------------------------------------------------------------"
);
//   3. Filter the elements of type: ‘fruit’ and add new property ‘isAvailable: true’
let filterFruit = superMarket.filter((fruits) => fruits.type === "fruit");
let addProperty = filterFruit.map((newFruit) => {
  return {
    ...newFruit,
    isAvailable: true,
  };
});
document.write(`<h2> Adding New Fruit Element </h2>`);
addProperty.forEach((newFruit) => {
  document.write(
    `-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------`
  );
  document.write(`<br>`);
  for (const key1 in newFruit) {
    document.write(`<b>${key1}</b>\t : ${newFruit[key1]}<br>`);
  }
});
/*4. Filter the elements of type: ‘dairy’ and ‘vegetable’ and add new property ‘isAvailable: true’
       to the filtered elements under type: ‘dairy’ and add new property ‘isAvailable: false’ to the
       filtered elements under type: ‘vegetable’ */
let newArray = superMarket
  .filter(
    (newFilterdArray) =>
      newFilterdArray.type === "dairy" || newFilterdArray.type === "vegetable"
  )
  .map((filteredArray) => {
    if (filteredArray.type === "dairy") {
      return {
        ...filteredArray,
        isAvailable: true,
      };
...
let addNewProperty = filterDairy.map((newDairy) => {
  return {
    ...newDairy,
    isAvailable: true,
  };
});
document.write(`<h2> Adding New Dairy Element </h2>`);
addNewProperty.forEach((newDairy) => {
  document.write(
    `-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------`
  );
  document.write(`<br>`);
  for (const key2 in newDairy) {
    document.write(`<b>${key2}</b>\t : ${newDairy[key2]}<br>`);
  }
});
let addNew2Property = filterVeg.map((newVeg) => {
  return {
    ...newVeg,
    isAvailable: false,
  };
});
document.write(`<h2> Adding New Veg Element </h2>`);
addNew2Property.forEach((newVeg) => {
  document.write(
    `-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------`
  );
  document.write(`<br>`);
  for (const key3 in newVeg) {
    document.write(`<b>${key3}</b>\t : ${newVeg[key3]}<br>`);
  }
});
